...
The source-data workbook in `source_data/Supplementary_Data_S1.xlsx` contains the derived tables that underlie the manuscript figures and tables. The script `scripts/validate_source_data.py` recomputes headline results directly from those tables and checks consistency across independently derived workbook tables.
